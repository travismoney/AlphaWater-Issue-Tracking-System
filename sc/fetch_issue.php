...
if(!isset($_SESSION['email_address'])){
    ...
    {
    $query = "
        SELECT * FROM issues WHERE assignee = '$user_id' ORDER BY 1 desc
        ";
    }
    ...
    while($row_project=mysqli_fetch_array($run_project)){
        ...
        $get_issued_by = "select * from users where user_id ='$issued_by' ";
        
        $run_issued_by = mysqli_query($con,$get_issued_by);
        
            while($row_issued_by=mysqli_fetch_array($run_issued_by)){
    
            $issued_by_first_name = $row_issued_by['first_name'];
    
            $issued_by_last_name = $row_issued_by['last_name'];
            ...
            if($row["priority"]=="1"){ // priority low
                ...
                <div class="row" style="border-bottom: 25px solid #f2f2f2 !important;">
                    ...
                    <div class="cell" data-title="Issued By">
                        '.$issued_by_first_name.' '.$issued_by_last_name.' 
                    </div>
                    ...
                    <div id="my-content" class="cell-hidden">
                        <a class="btn btn-success btn-block my-3 mx-4" href="update_progress?update_progress='.$row["issue_id"].'" role="button">Update Progress</a>
                    </div>
                    <div id="my-content" class="cell-hidden">
